...
namespace Nave { 
	...
	namespace Private 
	{
		#define MAKEQWORD(a, b)	\
			((Nave::int64)( ((Nave::int64) ((DWORD) (a))) << 32 | ((DWORD) (b))))

		#define LODWORD(l) \
			((DWORD)(l))
		#define HIDWORD(l) \
			((DWORD)(((Nave::int64)(l) >> 32) & 0xFFFFFFFF))
		...
		bool GetFileSizeQW(const HANDLE hFile, Nave::int64 &qwSize)
		{
	...
	DWORD NFChecksum::FromBuffer(BYTE* buffer, size_t size, DWORD &dwCrc32)
	{
		_ASSERTE(buffer);
	...
	DWORD NFChecksum::FromFileWin32(LPCWSTR filename, DWORD &dwCrc32)
	{
		_ASSERTE(filename);
	...
	DWORD NFChecksum::FromFileFilemap(LPCWSTR filename, DWORD &dwCrc32)
	{
		_ASSERTE(filename);
		_ASSERTE(lstrlenW(filename));
		...
		try
		{
			...
			{
				int64 qwFileSize = 0, qwFileOffset = 0;
	...
	DWORD NFChecksum::FromFileAssembly(LPCWSTR filename, DWORD &dwCrc32)
	{
		_ASSERTE(filename);
		_ASSERTE(lstrlenW(filename));
